...
void ItemRenderer::render(std::vector<Entity*> meshes, Camera* camera)
{
	...
	for (auto& chunkMesh : meshes) {
		model = glm::translate(model, chunkMesh->position);
		model = glm::rotate(model, -glm::radians(chunkMesh->getRotation()), glm::vec3(0, 1, 0));
		model = glm::scale(model, chunkMesh->scale);
